...
{
    internal sealed class PosixTerminalReader : ITerminalReader
    {
        ...
        public bool IsRedirected => !Syscall.isatty(PosixConstants.STDIN);
        ...
        public unsafe int Read(Span<byte> buffer)
        {
            ...
            while (true)
            {
                fixed (byte* p = buffer)
                {
                    while ((ret = Syscall.read(PosixConstants.STDIN, p, (ulong)buffer.Length)) == -1 &&
